# Ультимативная энциклопедия производительности 1С: Сложность алгоритмов, «вес» операций и методы рефакторинга

Этот справочник спроектирован как инструмент статического анализа legacy-кода. Оценка «веса» учитывает затраты процессора (CPU) сервера приложений, сетевой оверхед (RPC) и нагрузку на дисковую подсистему и пул потоков СУБД.

---

## 1. Слой ОЗУ: Встроенный язык и коллекции данных

Операции, выполняемые целиком в оперативной памяти сервера приложений. Опасность представляют скрытые линейные переборы больших массивов.


| Операция / Команда 1С | Временная сложность (Big O) | Локальный «Вес» | Соседи по весу (Аналоги) | Физическая суть («Под капотом») и методы оптимизации |
| :--- | :--- | :--- | :--- | :--- |
| **`Структура.Вставить()`** / **`.Получить()`** | $O(1)$ | **Невесомая** | Чтение переменной, доступ к свойству объекта. | Вычисление хэш-функции ключа-строки в ОЗУ. Поиск ячейки за один шаг. **Ограничение:** Не подходит для поиска по нескольким полям. |
| **`Соответствие.Вставить()`** / **`.Получить()`** | $O(1)$ | **Невесомая** | `Структура.Вставить()`. | Работает аналогично структуре, но ключом может быть любой ссылочный тип (`СправочникСсылка` и др.). **Идеально для замены вложенных циклов.** |
| **`Массив.Найти()`** | $O(N)$ | **Средняя** | Цикл `Для Каждого`. | Последовательный перебор элементов с нулевого индекса. Если в массиве $M$ элементов, а поиск вызывается в цикле по ТЧ из $N$ строк, сложность взлетает до $O(N \times M)$. **Лечение:** Выгрузить массив в `Соответствие` перед циклом. |
| **`ТЗ.НайтиСтроки()`** (Без индекса) | $O(N)$ | **Тяжелая** | Сканирование кучи. | Полный перебор всех строк таблицы значений в памяти. При росте таблицы время ответа растет линейно. |
| **`ТЗ.НайтиСтроки()`** (С индексом) | $O(\log N)$ | **Легкая** | Поиск по B-Tree индексу в СУБД. | Поиск по сбалансированному дереву индексов, созданному платформой в ОЗУ. **Решение:** Явно вызывать `ТЗ.Индексы.Добавить("ИмяКолонки")` перед массовым поиском. |
| **`ТЗ.Свернуть()`** | $O(N \log N)$ или $O(N)$ | **Тяжелая** | Сортировка массива, `ТЗ.Сортировать()`. | Платформа выделяет буфер памяти, производит внутреннюю сортировку или строит внутренний хэш-индекс для группировки строк. На миллионных ТЗ вызывает пиковую утилизацию CPU сервера приложений. |

---

## 2. Слой СУБД: Обращение к базе данных и неявные запросы

Самый критичный слой для Highload. Ошибки здесь приводят к блокировкам (`LCK_M_X`), падению пропускной способности и дисковому шторму.


| Операция / Команда 1С | Временная сложность (Big O) | Локальный «Вес» | Соседи по весу (Аналоги) | Физическая суть («Под капотом») и методы оптимизации |
| :--- | :--- | :--- | :--- | :--- |
| **Разыменование через точку (`Ссылка.Реквизит`)** | $O(1)$ в ОЗУ / $O(N)$ на СУБД | **Катастрофическая** | Запрос в цикле. | Если реквизит не был предварительно выбран запросом, платформа генерирует неявный `SELECT ... LEFT JOIN` к СУБД для получения данных. Вызов в цикле порождает лавину RPC-вызовов. **Лечение:** Заменить на явный пакетный запрос. |
| **Запрос к БД точно по индексам** | $O(\log N)$ | **Средняя** | `Index Seek` / `Index Scan` (в Postgres). | СУБД использует существующее B-Tree дерево индекса (например, поиск по `_IDRRef` или измерениям регистра сведений). Возвращает результат за фиксированное минимальное количество чтений. |
| **Запрос к БД без индексов (условие по неиндексированному полю)** | $O(N)$ | **Катастрофическая** | `Table Scan` / `Seq Scan`. | СУБД полностью игнорирует индексную страницу и начинает последовательно читать терабайты данных с диска в память. Блокирует параллельные потоки. **Лечение:** Добавить индексирование для измерения/реквизита в конфигураторе. |
| **Создание ВТ в цикле (`МенеджерВременныхТаблиц`)** | $O(N \times M)$ | **Катастрофическая** | Раздувание tempdb. | На каждой итерации цикла СУБД создает физическую временную таблицу в базе `tempdb`. Менеджер не очищается автоматически до конца работы модуля. Сессия пухнет, диски уходят в 100% утилизацию. **Лечение:** Формировать ВТ один раз, передавая массив параметров целиком. |
| **Использование конструкции `ГДЕ ... В (ВЫБРАТЬ ...)`** | От $O(\log N)$ до $O(N^2)$ | **Тяжелая** | Вложенный подзапрос. | Зависит от объема вложенной выборки. Оптимизатор СУБД может ошибиться с планом и переключить соединение в `Nested Loops` вместо `Hash Match`. **Решение:** Выносить вложенный запрос в отдельную ВТ. |

---

## 3. Слой транзакций и бизнес-логики: Запись данных

Операции изменения данных, которые невозможно упростить алгоритмически, так как они физически обязаны гарантировать целостность данных (ACID). Их можно только минимизировать.


| Операция / Команда 1С | Временная сложность (Big O) | Локальный «Вес» | Соседи по весу (Аналоги) | Физическая суть («Под капотом») и методы оптимизации |
| :--- | :--- | :--- | :--- | :--- |
| **`СправочникОбъект.Записать()`** | $O(N)$ | **Тяжелая** | `ДокументОбъект.Записать()`. | Открытие транзакции $\rightarrow$ Блокировка строки $\rightarrow$ Выполнение подписок на события $\rightarrow$ Запись в транзакционный лог (`WRITELOG` / WAL) $\rightarrow$ Обновление индексов таблицы. **Минимизация:** Использовать запись наборов записей или пакетирование. |
| **`РегистрНакопленияНаборЗаписей.Записать()`** | $O(N)$ | **Критическая** | Проведение документа. | Пересчет итогов регистра, проверка ограничений, запись движений. При RPS 15k+ вызов без флага `Замещение = Истина` вызывает предварительное чтение и удаление старых движений, удваивая нагрузку. **Минимизация:** Использовать разделение итогов. |

---

## 4. Памятка по рефакторингу Legacy-кода (Паттерны выявления антипаттернов)

При аудите старого кода используй метод поиска по ключевым словам. Ниже представлены три типичных маркера деградации и способы их переписывания.

### Маркер 1: Поиск дублей в ТЧ перебором (Квадратичный взрыв)
* **Legacy-код (Сложность $O(N^2)$):**
  ```bsl
  Для Каждого Стр1 Из ТЧТовары Цикл
      Для Каждого Стр2 Из ТЧТовары Цикл
          Если Стр1.Товар = Стр2.Товар И Стр1.НомерСтроки <> Стр2.НомерСтроки Тогда
              // Ошибка: дубль найден
          КонецЕсли;
      КонецЦикла;
  КонецЦикла;
  ```
* **Оптимальный код (Сложность $O(N)$):**
  ```bsl
  УникальныеТовары = Новый Соответствие;
  Для Каждого Стр Из ТЧТовары Цикл
      Если УникальныеТовары.Получить(Стр.Товар) <> Неопределено Тогда
          // Дубль найден за O(1) на шаге
      Иначе
          УникальныеТовары.Вставить(Стр.Товар, Истина);
      КонецЕсли;
  КонецЦикла;
  ```

### Маркер 2: Получение реквизитов через точку в цикле (RPC шторм СУБД)
* **Legacy-код (Сложность $O(N \times \text{Запрос})$):**
  ```bsl
  Для Каждого Строка Из ТЧ Цикл
      // Каждая точка генерирует неявный запрос к СУБД
      Артикул = Строка.Номенклатура.Артикул; 
      СтавкаНДС = Строка.Номенклатура.СтавкаНДС;
  КонецЦикла;
  ```
* **Оптимальный код (Сложность $O(1)$ в цикле):**
  ```bsl
  // Предварительно выбираем все данные одним пакетом через ВТ
  Запрос = Новый Запрос("ВЫБРАТЬ Номенклатура, Артикул, СтавкаНДС ИЗ Справочник.Номенклатура ГДЕ Ссылка В (&МассивНоменклатуры)");
  // Заполняем Соответствие данными и обходим ТЧ, читая только из памяти
  ```

### Маркер 3: Поиск в неиндексированной таблице значений
* **Legacy-код (Сложность $O(N \times M)$):**
  ```bsl
  Для Каждого Заказ Из МассивЗаказов Цикл
      // Платформа делает полный перебор строк таблицы значений на каждой итерации
      СтрокаТЗ = КэшДанных.Найти(Заказ, "ДокументЗаказ"); 
  КонецЦикла;
  ```
* **Оптимальный код (Сложность $O(N \times \log M)$):**
  ```bsl
  // Перед циклом строим B-Tree индекс в ОЗУ сервера приложений
  КэшДанных.Индексы.Добавить("ДокументЗаказ");
  
  Для Каждого Заказ Из МассивЗаказов Цикл
      // Поиск переключается в логарифмический режим
      Найденные = КэшДанных.НайтиСтроки(Новый Структура("ДокументЗаказ", Заказ));
  КонецЦикла;
  ```
